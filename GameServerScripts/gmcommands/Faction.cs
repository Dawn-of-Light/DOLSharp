		...
		public int OnCommand(GameClient client, string[] args)
		{
			...
			switch (args[1])
			{
				case "create":
				{
					...
					try
					{
						basefriendship = Convert.ToInt32( args[3]);
					}
					catch
					{
				...
				case "assign":
				{
					...
					if (npc == null)
					{
						client.Player.Out.SendMessage("You must select a mon first.",eChatType.CT_Say,eChatLoc.CL_SystemWindow);
						return 0;
					}
					npc.Faction = myfaction;
					client.Player.Out.SendMessage("The mob " + npc.Name + "have join the faction of "+myfaction.Name+".",eChatType.CT_Say,eChatLoc.CL_SystemWindow);
				}break;
				case "addfriend":
				{
					if ( myfaction == null)
				...
				case "list":
				{
					foreach (Faction faction in  FactionMgr.Factions.Values)
						client.Player.Out.SendMessage("#"+faction.ID.ToString()+": "+faction.Name,eChatType.CT_Say,eChatLoc.CL_SystemWindow);
					return 1;
				}
				case "select":
				{
