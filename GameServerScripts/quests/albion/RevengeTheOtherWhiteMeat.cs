	...
	public class RevengeTheOtherWhiteMeat : BaseQuest
	{
				...
				if (quest == null)
				{
					...
					switch (wArgs.Text)
					{
						...
						case "like that":
							...
							player.Out.SendCustomDialog("Will you help Farmer Asma get \nrevenge on the pig herders?\n[Level 5-8]", new CustomDialogResponse(CheckPlayerAcceptQuest));
							break;
					}
				}
				else
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!QuestMgr.GiveQuestToPlayer(typeof(RevengeTheOtherWhiteMeat), player, farmerAsma))
					return;
