...
*/
     ...
     */
    [NHibernate.Mapping.Attributes.Subclass(NameType = typeof(ArrowsForYettaFletcher), ExtendsType = typeof(AbstractQuest))] 
	public class ArrowsForYettaFletcher : BaseQuest
	{
		...
		public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			*/
			...
			// item db check
			bundleOfDecayedZombieLegs = (GenericItemTemplate) GameServer.Database.FindObjectByKey(typeof (GenericItemTemplate), "bundle_of_decayed_zombie_legs");
			if (bundleOfDecayedZombieLegs == null)
			{
				...
				bundleOfDecayedZombieLegs.ItemTemplateID = "bundle_of_decayed_zombie_legs";
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
                ...
                if (!QuestMgr.GiveQuestToPlayer(typeof(ArrowsForYettaFletcher), player, yettaFletcher))
					return;
			...
			if (e == GameLivingEvent.EnemyKilled)
			{
				if(Step == 2)
				{
					...
					if (gArgs.Target.Name == "decayed zombie")
					{
						if (Util.Chance(50))
						{
                            GiveItemToPlayer(gArgs.Target, CreateQuestItem(bundleOfDecayedZombieLegs, Name));
							Step = 3;
						}
						return;
					}
				...
				else if(Step == 3)
				{
					...
					if (gArgs.Target.Name == "decayed zombie")
					{
						if (Util.Chance(50))
						{
							GiveItemToPlayer(gArgs.Target, bundleOfDecayedZombieLegs.CreateInstance());
							Step = 4;
						}
						return;
					}
