...
namespace DOL.GS.Quests.Albion
{
    ...
    */
     ...
     */
    [Subclass(NameType = typeof(GodelevasNeed), ExtendsType = typeof(AbstractQuest))]
	public class GodelevasNeed : BaseQuest
	{
		...
		private static GameMob godelevaDowden = null;
		...
		public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			*/
			...
			godelevaDowden = ResearchQuestObject(typeof(GameMob), WorldMgr.GetRegion(1), eRealm.Albion, "Godeleva Dowden") as GameMob;
			if (godelevaDowden == null)
			{
				...
				godelevaDowden.Region = WorldMgr.GetRegion(1);
				...
				godelevaDowden.Inventory = template;
				...
				newBrain.Body = godelevaDowden;
				newBrain.AggroLevel = 100;
				newBrain.AggroRange = 0;
				godelevaDowden.OwnBrain = newBrain;

				if(!godelevaDowden.AddToWorld())
				{
					if (log.IsWarnEnabled)
				...
				if (SAVE_INTO_DATABASE)
					GameServer.Database.AddNewObject(godelevaDowden);
			}
			...
			// item db check
			woodenBucket = GameServer.Database.SelectObject(typeof (GenericItemTemplate), Expression.Eq("Name", "Wooden Bucket")) as GenericItemTemplate;
			...
			// item db check
			fullWoodenBucket = GameServer.Database.SelectObject(typeof (GenericItemTemplate), Expression.Eq("Name", "Full Wooden Bucket")) as GenericItemTemplate;
			...
			// item db check
			reedBracer = GameServer.Database.SelectObject(typeof (BracerTemplate), Expression.Eq("Name", "Reed Bracer")) as BracerTemplate;
			if (reedBracer == null)
			{
				if (log.IsWarnEnabled)
			...
			*/
			...
			GameEventMgr.AddHandler(godelevaDowden, GameObjectEvent.Interact, new DOLEventHandler(TalkToGodelevaDowden));
		...
		public static void ScriptUnloaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			GameEventMgr.RemoveHandler(godelevaDowden, GameObjectEvent.Interact, new DOLEventHandler(TalkToGodelevaDowden));
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					switch (wArgs.Text)
					{
						...
						case "fetch":
							QuestMgr.ProposeQuestToPlayer(typeof(GodelevasNeed), "Will you take the bucket\ndown to the river and\nretrieve some water for\nGodeleva?", player, godelevaDowden);
							break;
					}
				}
			}
		...
		protected static void PlayerUseSlot(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (quest.Step == 1)
			{
				...
				if (item != null && item.QuestName == quest.Name && item.Name == woodenBucket.Name)
				{
					if (player.Position.CheckSquareDistance(cotswoldVillageBridge.Position, 500*500))
					{
						...
						GiveItemToPlayer(CreateQuestItem(fullWoodenBucket, quest), player);
		...
		protected static void QuestDialogResponse(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (gArgs != null && gArgs.QuestType.Equals(typeof(GodelevasNeed)))
			{
				...
				{
					if (QuestMgr.GiveQuestToPlayer(typeof(GodelevasNeed), player, gArgs.Source as GameNPC))
					{
						// give woodenBucket 
						GiveItemToPlayer(CreateQuestItem(woodenBucket, player.IsDoingQuest(typeof(GodelevasNeed))), player);
		...
		public override void Notify(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (e == GamePlayerEvent.GiveItem)
			{
				if(Step == 2)
				{
					...
					if (gArgs.Target == godelevaDowden && gArgs.Item.QuestName == Name && gArgs.Item.Name == fullWoodenBucket.Name)
					{
