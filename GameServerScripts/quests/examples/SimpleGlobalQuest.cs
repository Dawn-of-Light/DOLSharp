...
namespace DOL.GS.Quests.Examples
{
	 ...
	 */
	[Subclass(NameType=typeof(SimpleGlobalQuest), ExtendsType=typeof(AbstractQuest))] 
	public class SimpleGlobalQuest : BaseQuest
	{
		...
		public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
		{
			...
			#region Define NPC and ItemTemplate
				...
				/* If evilSpider == null then no Evil Spider exists in
				* this users Mob Database, so we simply create one ;-)
				* else we take the existing one.
				*/
			evilSpider = ResearchQuestObject(typeof(GameMob), WorldMgr.GetRegion(1), eRealm.None, "The evil spider") as GameMob;
			if (evilSpider == null)
			{
				...
				evilSpider.Region = WorldMgr.GetRegion(1);
				evilSpider.Size = 90;
				evilSpider.Level = 3;
				...
				newBrain.Body = evilSpider;
				newBrain.AggroLevel = 100;
				newBrain.AggroRange = 1000;
				evilSpider.OwnBrain = newBrain;

				if(!evilSpider.AddToWorld())
				{
					if (log.IsWarnEnabled)
						log.Warn("SimpleGlobalQuest quest abort because a needed region is not in use in this server!");
					return;
				}
		...
		protected static void QuestDialogReponse(DOLEvent e, object sender, EventArgs args)
		{
			...
			if(gArgs != null && gArgs.QuestType.Equals(typeof (SimpleGlobalQuest)))
			{
				...
				{
					if(QuestMgr.GiveQuestToPlayer(typeof (SimpleGlobalQuest), player))
					{
						player.Out.SendMessage("Well, it is very simple. Just kill the evil spider (Albion loc : [534000,478955]) and you will have a reward.", eChatType.CT_Say, eChatLoc.CL_PopupWindow);
					}
				}
		...
		protected static void QuestCancelDialogResponse(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (gArgs != null && gArgs.Quest.GetType().Equals(typeof(SimpleGlobalQuest)))
			{
				GamePlayer player = gArgs.Player;
				if (player == null) return;
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return "[Step #1] Kill the evil spider! (Albion loc : [534000,478955])";
                    default:
