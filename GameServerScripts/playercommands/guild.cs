...
namespace DOL.GS.Scripts
{
	...
	public class GuildCommandHandler : ICommandHandler
	{
                    ...
                    case "who":
                        {
                            ...
                            if (args.Length == 6 && args[2] == "window" && (client.Player.Guild != null))
                            {
                                ...
                                for (byte i = 0; i < maxShowed; i++)
                                {
                                    ...
                                    (i + 1), 0, ply.Name, ply.Level, ply.CharacterClass.ID, ply.GuildRank, (ply.PlayerGroup == null ? 1 : 2), (ply.CurrentZone == null ? "" : ply.CurrentZone.Description), ""/*Note*/), eChatType.CT_SocialInterface, eChatLoc.CL_SystemWindow);
                                }
                                return 1;
                            }
        ...
        public static void EmblemChange(GamePlayer player, byte response)
        {
            ...
            {
                player.Client.Out.SendMessage("You must select the Guild Emblemeer before accepting this dialog.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            if (player.GetCurrentMoney() < GuildMgr.COST_RE_EMBLEM) //200 gold to re-emblem
            {
                player.Out.SendMessage("You do not have enough gold to change your guild's emblem.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            player.RemoveMoney(GuildMgr.COST_RE_EMBLEM, null);
