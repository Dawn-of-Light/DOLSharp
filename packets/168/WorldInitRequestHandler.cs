		...
		protected class WorldInitAction : RegionAction
		{
			...
			protected override void OnTick()
			{
				...
				foreach(eInventorySlot slot in GameLivingInventory.EQUIP_SLOTS)
					if(player.Inventory.GetItem(slot) != null) slotToUpdate.Add((int)slot);
				
				player.Out.SendInventorySlotsUpdate(0x01, slotToUpdate); // update equiped item
				...
				player.Out.SendInventorySlotsUpdate(0x02, slotToUpdate); // update backpack
				...
				if (log.IsDebugEnabled)
					log.DebugFormat("Client {0}({1} PID:{2} OID:{3}) entering Region {4}(ID:{5})", player.Client.Account.AccountName, player.Name, player.Client.SessionID, player.ObjectID, player.Region.Description, player.Region.RegionID);
			}
		}
	}
}