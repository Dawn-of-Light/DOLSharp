	...
	public class MoneyTask : AbstractTask
	{
		...
		public static bool BuildTask(GamePlayer player, GameLiving source)
		{
			...
			else
			{
				...
				((MoneyTask)player.Task).RecieverZone = NPC.Region.GetZone(NPC.Position).Description;
				
				player.Out.SendMessage("Bring "+TaskItems.Name+" to "+NPC.Name +" in "+ NPC.Region.GetZone(NPC.Position).Description, eChatType.CT_Say, eChatLoc.CL_PopupWindow);
