    ...
    public class BaseQuestTrigger : AbstractQuestTrigger
    {
                ...
                case eTriggerType.GiveItem:
                    {
						...
						if (I is string)
							I = (GenericItemTemplate)GameServer.Database.FindObjectByKey(typeof(GenericItemTemplate), Convert.ToString(I));
                        if (!(I is GenericItemTemplate))
                ...
                case eTriggerType.GiveItem:
                    {
                        if (e == GamePlayerEvent.GiveItem)
                        {
                            ...
                            GenericItemTemplate item = I as GenericItemTemplate;
                ...
                case eTriggerType.AcceptQuest:
                    {
                        if (e == GamePlayerEvent.AcceptQuest)
                        {
                            ...
                            result |= (qArgs.Player.ObjectID == player.ObjectID && qArgs.QuestDescriptor.LinkedQuestType == questType);
                        }
                        break;
                    }
                case eTriggerType.DeclineQuest:
                ...
                case eTriggerType.ItemUsed:
                    {
                        ...
                        {
                            GenericItemTemplate itemTemplate = (GenericItemTemplate)I;
        ...
        public override void Register()
        {
            ...
            switch (TriggerType)
            {
                ...
                case eTriggerType.Whisper:
					...
					GameEventMgr.AddHandlerUnique(NPC, GameLivingEvent.Interact, NotifyHandler);
                    break;
                case eTriggerType.AcceptQuest:
        ...
        public override void Unregister()
        {
            ...
            switch (TriggerType)
            {
                ...
                case eTriggerType.Whisper:
					...
					GameEventMgr.RemoveHandler(NPC, GameLivingEvent.Interact, NotifyHandler);
                    break;
                case eTriggerType.AcceptQuest:
		...
		public override string ToString()
		{
			...
			switch (TriggerType)
			{
				...
				case eTriggerType.GiveItem:
					{
						text = String.Format("the player gives {0} to {1}", new object[] { ((GenericItemTemplate)I).Name, (I != null ? ((GameLiving)I).Name : NPC.Name) });												
						break;
					}
				case eTriggerType.AcceptQuest:
					{
				...
				case eTriggerType.EnterArea:
					{
						text = String.Format("the player enters {0}", new object[] { (I is AbstractArea && !String.IsNullOrEmpty(((AbstractArea)I).Description)) ? ((AbstractArea)I).Description : "an area" });						
						break;
					}
				case eTriggerType.LeaveArea:
					{
						text = String.Format("the player leaves {0}", new object[] { (I is AbstractArea && !String.IsNullOrEmpty(((AbstractArea)I).Description)) ? ((AbstractArea)I).Description : "an area" });						
						break;
					}
				case eTriggerType.Timer:
					{
				...
				case eTriggerType.ItemUsed:
					{
						text = String.Format("the player uses {0}", new object[] { ((GenericItemTemplate)I).Name });
						break;
					}
			}
