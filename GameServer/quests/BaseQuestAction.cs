...
namespace DOL.GS.Quests
{
	...
	public class ActionTypeAttribute : Attribute
	{
		...
		public ActionTypeAttribute(eActionType actionType,string name,string description,string intellitext)
		{
	...
	{		
		public ParameterAttribute(eSelectorType selectorType,Type contentType, object defaultValue)
		{
		
		}
	}

	public class ParameterQAttribute : ParameterAttribute
	{
		public ParameterQAttribute(eSelectorType selectorType, Type contentType, object defaultValue) : base(selectorType,contentType,defaultValue)	{ }
	}

	public class ParameterPAttribute : ParameterAttribute
	{
		public ParameterPAttribute(eSelectorType selectorType, Type contentType, object defaultValue) : base(selectorType,contentType,defaultValue)	{ }
    ...
    public class BaseQuestAction : AbstractQuestAction
    {
        ...
        {            
            switch (ActionType)
            {
                ...
                case eActionType.DestroyItem:
                    {
						if (P is string)
							P = (GenericItemTemplate)GameServer.Database.FindObjectByKey(typeof(GenericItemTemplate), Convert.ToString(P));

                        if (!(P is GenericItemTemplate))
                ...
                case eActionType.ReplaceItem:
                    {
						...
						if (Q is string)
							Q = (GenericItemTemplate)GameServer.Database.FindObjectByKey(typeof(GenericItemTemplate), Convert.ToString(Q));

						if (!(Q is GenericItemTemplate))
		...
		[ActionType(eActionType.IncQuestStep,
			"Increase Queststep",
			"Increaes the current queststep by 1",
			"the queststep of $P{0}$ is increased"),
		ParameterQ(eSelectorType.QuestType,typeof(String),null)]
		public static bool IncreaseQuestStep(GamePlayer player, Type questType)
		{
			...
			if (playerQuest != null)
			{
				playerQuest.ChangeQuestStep(playerQuest.Step++);
				return true;
			}
			else
			{
		...
		public static bool SetQuestStep(GamePlayer player, Type questType, byte step)
		{
			...
			if (playerQuest != null)
			{
				playerQuest.ChangeQuestStep(step);
				return true;
			}
			else
				return false;
		...
		public static bool GiveItem(GamePlayer player, GenericItem item, GameNPC npc)
		{
		...
		public static bool ReplaceItem(GamePlayer player, GenericItem oldItem, GenericItem newItem)
		{
		...
		public static bool TakeItem(GamePlayer player, GenericItem itemToRemove, int amount, GameNPC npc)
		{
			lock (player.Inventory)
			{
				GenericItem item = player.Inventory.GetFirstItemByName(itemToRemove.Name, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
				//remove item/s from player
				while (amount > 0 && item != null)
				{
					StackableItem itemStack = item as StackableItem;
					if (itemStack != null)
					{
						if (itemStack.Count > amount)
						{
							player.Inventory.RemoveCountFromStack(itemStack, amount);
							amount = 0;
						}
						else
						{
					...
					item = player.Inventory.GetFirstItemByName(itemToRemove.Name, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
				}
		...
		public static bool DropItem(GamePlayer player, GenericItem item)
		{
		...
		public static bool DestroyItem(GamePlayer player, GenericItem itemToDestroy, int amount)
		{
			lock (player.Inventory)
			{
				GenericItem item = player.Inventory.GetFirstItemByName(itemToDestroy.Name, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
				//remove item/s from player
				while (amount > 0 && item != null)
				{
					StackableItem itemStack = item as StackableItem;
					...
					item = player.Inventory.GetFirstItemByName(itemToDestroy.Name, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
				}
			}
		...
		public static bool Teleport(GamePlayer player, GameLocation location, int radius) {
        	...
        	pos.X += Util.Random(-radius, radius);
        	pos.Y += Util.Random(-radius, radius);
            return player.MoveTo((ushort)location.Region.RegionID, pos, location.Heading);			
		}
        ...
        public override void Perform(DOLEvent e, object sender, EventArgs args, GamePlayer player)
        {
            switch (ActionType)
            {
                ...
                case eActionType.GiveItem:
                    {
                        GenericItem item = (GenericItem)P;
                ...
                case eActionType.ReplaceItem:
                    {
                       GenericItem newItem = (GenericItem)Q;
                       GenericItem oldItem = (GenericItem)P;
                ...
                case eActionType.TakeItem:
                    {
                        GenericItem itemToRemove = (GenericItem)P;
                ...
                case eActionType.DestroyItem:
                    {
                        GenericItem itemToDestroy = (GenericItem)P;
                        int amount = Q != null ? Convert.ToInt32(Q) : 1;
                ...
                case eActionType.WalkTo:
                    {
                        ...
                        Point location = (P is Point) ? (Point)P : player.Position;
				...
				case eActionType.MoveTo:
					{
						...
						if (P is GameLocation)
						{
							...
							npc.MoveTo((ushort)location.Region.RegionID, location.Position, location.Heading);
						}
						else
						{
							npc.MoveTo((ushort)player.Region.RegionID, player.Position, (ushort)player.Heading);							
						}						
