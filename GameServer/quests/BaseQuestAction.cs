...
namespace DOL.GS.Quests
{
    ...
    public class BaseQuestAction : AbstractQuestAction
    {
		...
		ParameterQ(eSelectorType.QuestType, typeof(String), "")]
		...
		public static bool TakeItem(GamePlayer player, GenericItem itemToRemove, int count, GameNPC npc)
		{
		...
		public static bool DestroyItem(GamePlayer player, GenericItem itemToDestroy, int count)
		{	
			Hashtable dataSlots = new Hashtable(10);
			lock(player.Inventory)
			{
		...
		{
			foreach (GamePlayer nearPlayer in actor.GetInRadius(typeof(GamePlayer), WorldMgr.VISIBILITY_DISTANCE))
			{
		...
		public static bool Teleport(GamePlayer player, GameLocation location, int radius) {
            ...
            return player.MoveTo(location.Region, pos, location.Heading);			
		}
		...
		public static bool MonsterSpawn(GameLiving living)
		{
			if (living.AddToWorld())
			{
				...
				foreach (GamePlayer visPlayer in living.GetInRadius(typeof(GamePlayer), WorldMgr.VISIBILITY_DISTANCE))
				{
				...
				case eActionType.MoveTo:
					{
						...
						if (P is GameLocation)
						{
							...
							npc.MoveTo(location.Region, location.Position, location.Heading);
						}
						else
						{
							npc.MoveTo(player.Region, player.Position, (ushort)player.Heading);							
						}						
