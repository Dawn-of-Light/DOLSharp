...
namespace DOL.GS
{
    public class SpellCrafting : AdvancedCraftingSkill
    {
        ...
        {
            Icon = 0x0D;
            Name = LanguageMgr.GetTranslation(
        ...
        {
            if (item == null || player.TradeWindow.TradeItems[0] == null)
            {
                return; // be sure at least one item in each side
            }
        ...
        {
            if (((InventoryItem)player.TradeWindow.TradeItems[0]).Model == 525)
            {
                return; // check if applying dust
            }
        ...
        protected int GetTotalImbuePoints(GamePlayer player, InventoryItem item)
        {
            ...
            lock (player.TradeWindow.Sync)
            {
                for (int i = 0; i < player.TradeWindow.TradeItems.Count; i++)
                {
                    ...
                    if (currentGemCharge > biggerGemCharge)
                    {
                        biggerGemCharge = currentGemCharge;
                    }
        ...
        {
            int gemBonus;
            if (bonusType <= (int)eProperty.Stat_Last)
            {
                gemBonus = (int)(((bonusValue - 1) * 2 / 3) + 1); // stat
            }
            else if (bonusType == (int)eProperty.MaxMana)
            {
                gemBonus = (int)((bonusValue * 2) - 2); // mana
            }
            else if (bonusType == (int)eProperty.MaxHealth)
            {
                gemBonus = (int)(bonusValue / 4); // HP
            }
            else if (bonusType <= (int)eProperty.Resist_Last)
            {
                gemBonus = (int)((bonusValue * 2) - 2);// resist
            }
            else if (bonusType <= (int)eProperty.Skill_Last)
            {
                gemBonus = (int)((bonusValue - 1) * 5);// skill
            }
            else
            {
                gemBonus = 1;// focus
