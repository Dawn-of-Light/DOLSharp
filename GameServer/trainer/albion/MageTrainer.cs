	...
	public class MageTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			if (player.CharacterClass.ID == (int)TrainedClass)
			{
				...
				if (player.Level>=5)
				{
					player.Out.SendMessage(Name + " says, \"You must now seek your training elsewhere. Which path would you like to follow? [Cabalist] or [Sorcerer]?\"", eChatType.CT_Say, eChatLoc.CL_PopupWindow);
				}
				else
				{
			...
			switch (text) {
				case "Cabalist":
					if(player.Race == (int) eRace.Avalonian || player.Race == (int) eRace.Briton || player.Race == (int) eRace.HalfOgre || player.Race == (int) eRace.Inconnu || player.Race == (int) eRace.Saracen){
						...
						player.Out.SendMessage(Name + " says, \"The path of a Cabalist is not available to your race. Please choose another.\"",eChatType.CT_Say,eChatLoc.CL_PopupWindow);
					}
					return true;
				case "Sorcerer":
					if(player.Race == (int) eRace.Briton || player.Race == (int) eRace.Avalonian || player.Race == (int) eRace.HalfOgre || player.Race == (int) eRace.Inconnu || player.Race == (int) eRace.Saracen){
						...
						player.Out.SendMessage(Name + " says, \"The path of a Sorcerer is not available to your race. Please choose another.\"",eChatType.CT_Say,eChatLoc.CL_PopupWindow);
					}
					return true;
