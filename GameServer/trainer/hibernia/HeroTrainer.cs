...
namespace DOL.GS.Trainer
{
	...
	public class HeroTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			{
				player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "HeroTrainer.Interact.Text2", Name, player.GetName(0, false)), eChatType.CT_Say, eChatLoc.CL_ChatWindow);
			}
			else
			{
				...
				{
					player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "HeroTrainer.Interact.Text1", Name), eChatType.CT_System, eChatLoc.CL_PopupWindow);
