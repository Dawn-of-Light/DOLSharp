    ...
    public class StandardMobBrain : APlayerVicinityBrain, IOldAggressiveBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.MaxSpeedBase > 0 && Body.CurrentSpellHandler == null && !Body.IsMoving
                ...
                && Body.PathID != null && Body.PathID != string.Empty && Body.PathID != "NULL")
            {
        ...
        public virtual void AddToAggroList(GameLiving living, int aggroamount, bool NaturalAggro)
        {
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType<ProtectEffect>())
                {
                    ...
                    if (protect.ProtectSource.IsStunned)
                    {
                        continue;
                    }

                    if (protect.ProtectSource.IsMezzed)
                    {
                        continue;
                    }
                    ...
                    if (protect.ProtectSource.ObjectState != GameObject.eObjectState.Active)
                    {
                        continue;
                    }

                    if (!protect.ProtectSource.IsAlive)
                    {
                        continue;
                    }
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(
                            LanguageMgr.GetTranslation(protect.ProtectSource.Client.Account.Language, "AI.Brain.StandardMobBrain.YouProtDist", player.GetName(0, false),
        ...
        public override void Notify(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (sender == Body)
            {
                if (e == GameObjectEvent.TakeDamage)
                {
                    ...
                    if (eArgs == null || eArgs.DamageSource is GameLiving == false)
                    {
                        return;
                    }
            ...
            if (Body != null && Body.Spells != null && Body.Spells.Count > 0)
            {
                ...
                if (type == eCheckSpellType.Defensive)
                {
                    foreach (Spell spell in Body.Spells)
                    {
                        ...
                        {
                            if (spell.SpellType.ToLower() == "pet")
                            {
                                continue;
                            }
                        ...
                        if (Body.ControlledBrain != null && Body.ControlledBrain.Body != null)
                        {
                            ...
                            if (LivingHasEffect(Body.ControlledBrain.Body, spell) && (spell.Target.ToLower() != "self"))
                            {
                                continue;
                            }
                        }
