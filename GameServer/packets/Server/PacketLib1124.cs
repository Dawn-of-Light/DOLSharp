...
namespace DOL.GS.PacketHandler
{
    ...
    public class PacketLib1124 : AbstractPacketLib, IPacketLib
    {
        ...
        public virtual void SendBlinkPanel(byte flag)
		{
			...
			{
				GamePlayer player = base.GameClient.Player;
        ...
        public virtual void SendCharStatsUpdate()
		{
			...
			int bonusCap = (int)(GameClient.Player.Level/2 + 1);
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.StatsUpdate)))
			{
				...
				for (int i = 0; i < updateStats.Length; i++)
				{
					...
					pak.WriteShort( (ushort)(GameClient.Player.ItemBonus[(int)updateStats[i]] + acuityItemBonus) );
				}
				...
				{
					int acuityItemBonus = 0;
					if (GameClient.Player.CharacterClass.ClassType != eClassType.PureTank && (int)updateStats[i] == (int)GameClient.Player.CharacterClass.ManaStat)
					{
						if (GameClient.Player.CharacterClass.ID != (int)eCharacterClass.Scout && GameClient.Player.CharacterClass.ID != (int)eCharacterClass.Hunter && GameClient.Player.CharacterClass.ID != (int)eCharacterClass.Ranger)
						{
					...
					pak.WriteByte((byte)(GameClient.Player.AbilityBonus[(int)updateStats[i]] + acuityItemBonus));
				}
				...
				//pak.Fill(0, 9);
				//if (GameClient.Player.CharacterClass.ID == (int)eCharacterClass.Vampiir)
				//	pak.WriteByte((byte)(GameClient.Player.Level - 5)); // Vampire bonuses
				//else
		...
		{
			//ChatUtil.SendDebugMessage(GameClient, string.Format("SendItemsPartialUpdate: windowType: {0}, {1}", windowType, items == null ? "nothing" : items[0].Name));
		...
		protected virtual void SendInventorySlotsUpdateRange(ICollection<int> slots, eInventoryWindowType windowType)
		{
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.InventoryUpdate)))
			{
				...
				pak.WriteByte((byte)GameClient.Player.VisibleActiveWeaponSlots);
				pak.WriteByte((byte)windowType);
        ...
        public virtual void SendMasterLevelWindow(byte ml)
		{
			...
			{
				mlXPPercent = 100.0 * (double)GameClient.Player.MLExperience / (double)GameClient.Player.GetMLExperienceForLevel((int)(GameClient.Player.MLLevel + 1));
				if (GameClient.Player.GetStepCountForML((byte)(GameClient.Player.MLLevel + 1)) > 0)
				{
					mlStepPercent = 100.0 * (double)GameClient.Player.GetCountMLStepsCompleted((byte)(GameClient.Player.MLLevel + 1)) / (double)GameClient.Player.GetStepCountForML((byte)(GameClient.Player.MLLevel + 1));
				}
			}
			else
			{
        ...
        public virtual void SendNPCCreate(GameNPC npc)
		{
            ...
            if (npc is GameMine)
			{
				if (GameServer.ServerRules.IsAllowedToAttack((npc as GameMine).Owner, GameClient.Player, true))
				{
					return;
				}
			}
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.NPCCreate)))
			{
				...
				LanguageDataObject translation = LanguageMgr.GetTranslation(GameClient, npc);
				if (translation != null)
				{
            ...
            if (GameClient.Player.Client.Version < GameClient.eClientVersion.Version1124) 
            {   // Update Cache
		...
		public virtual void SendObjectCreate(GameObject obj)
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ObjectCreate)))
			{
				...
				if (obj is Keeps.GameKeepBanner)
					flag |= 0x08;
				if (obj is GameStaticItemTimed && GameClient.Player != null && ((GameStaticItemTimed)obj).IsOwner(GameClient.Player))
					flag |= 0x04;
	            ...
	            if (obj is GameStaticItem)
	            {
	                translation = LanguageMgr.GetTranslation(GameClient, (GameStaticItem)obj);
	                if (translation != null)
	                {
		...
		public virtual void SendObjectUpdate(GameObject obj)
		{
			...
			if (z == null ||
				...
				GameClient.Player.IsVisibleTo(obj) == false)
			{
				return;
			}
        ...
        public virtual void SendPlayerCreate(GamePlayer playerToCreate)
        {
            ...
            {
                if (log.IsErrorEnabled)
                    log.Error("SendPlayerCreate: GameClient.Player == null");
                return;
            }
		...
		public virtual void SendPlayerPositionAndObjectID()
        {
            ...
            {
                pak.WriteFloatLowEndian(GameClient.Player.X);
                pak.WriteFloatLowEndian(GameClient.Player.Y);
                pak.WriteFloatLowEndian(GameClient.Player.Z);
		...
		public virtual void SendPlayerQuit(bool totalOut)
		{
			using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.Quit)))
			{
				...
				if (GameClient.Player == null)
					...
					pak.WriteByte(GameClient.Player.Level);
				SendTCP(pak);
			}
		}
		public virtual void SendPlayerTitles()
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.DetailWindow)))
			{
				...
				if (titlesLen > byte.MaxValue)
					log.WarnFormat("Titles block is too long! {0} (player: {1})", titlesLen, GameClient.Player);
		...
		public virtual void SendQuestUpdate(AbstractQuest quest)
		{
			...
			if (GameClient != null && GameClient.Player != null && GameClient.Player.QuestList != null)
			{
			...
			if (q == null)// || !(q is RewardQuest))
			{
				using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.QuestEntry)))
				{
					...
					else
					{
						...
						{
							if (log.IsWarnEnabled) log.Warn(q.GetType().ToString() + ": name is too long for 1.68+ clients (" + name.Length + ") '" + name + "'");
							name = name.Substring(0, byte.MaxValue);
						}
						if (desc.Length > byte.MaxValue)
						{
							if (log.IsWarnEnabled) log.Warn(q.GetType().ToString() + ": description is too long for 1.68+ clients (" + desc.Length + ") '" + desc + "'");
		...
		public virtual void SendSiegeWeaponInterface(GameSiegeWeapon siegeWeapon, int time)
		{
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.SiegeWeaponInterface)))
			{
	            ...
	            LanguageDataObject translation = LanguageMgr.GetTranslation(GameClient, siegeWeapon);
	            if (translation != null)
	            {
	                if (!Util.IsEmpty(((DBLanguageNPC)translation).Name))
	                    name = ((DBLanguageNPC)translation).Name;
		...
		public virtual void SendStatusUpdate(byte sittingFlag)
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterStatusUpdate)))
			{
				...
				//			pak.WriteShort((byte) (GameClient.Player.IsAlive ? 0x00 : 0x0f)); // 0x0F if dead ??? where it now ?
				pak.WriteByte(0);// unk
		...
		public virtual void SendTradeWindow()
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TradeWindow)))
			{
				lock (GameClient.Player.TradeWindow.Sync)
				{
					...
					if (GameClient.Player.TradeWindow.Partner != null)
						pak.WritePascalString("Trading with " + GameClient.Player.GetName(GameClient.Player.TradeWindow.Partner)); // transaction with ...
					else
		...
		public virtual void SendTrainerWindow()
		{
			...
			var raList = SkillBase.GetClassRealmAbilities(GameClient.Player.CharacterClass.ID).Where(ra => !(ra is RR5RealmAbility));
			using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
			{
			...
			using(GSTCPPacketOut pakskill = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
			{
				...
				for (byte i = 2; i <= 50; i++)
				{
					...
					//if (i > 5)
					//    specpoints = i * GameClient.Player.CharacterClass.AdjustedSpecPointsMultiplier / 10;
	
					//if (i > 40 && i != 50)
					//    specpoints += i * GameClient.Player.CharacterClass.AdjustedSpecPointsMultiplier / 20;
			...
			using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
			{
				...
				foreach (RealmAbility ra in raList)
				{
					...
					if (ra.CheckRequirement(GameClient.Player))
			...
			foreach (Skill t in skills)
			{
				...
				if (t is RealmAbility)
				{
					if (GameClient.CanSendTooltip(27, t.InternalID))
						SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveRealmAbility(GameClient, t.InternalID));
				}
				else if (t is Ability)
				{				
					if (GameClient.CanSendTooltip(28, t.InternalID))
						SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveAbility(GameClient, t.InternalID));
				}
				else if (t is Style)
				{
					if (GameClient.CanSendTooltip(25, t.InternalID))
						SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveStyle(GameClient, t.InternalID));
				}
				else if (t is Spell)
				{
					if (t is Song || (t is Spell && ((Spell)t).NeedInstrument))
					{
						if (GameClient.CanSendTooltip(26, ((Spell)t).InternalID))
							SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveSong(GameClient, ((Spell)t).InternalID));
					}
					
					if (GameClient.CanSendTooltip(24, ((Spell)t).InternalID))
						SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveSpell(GameClient, ((Spell)t).InternalID));
				}
			}
		}
		...
		public virtual void SendUpdateIcons(IList changedEffects, ref int lastUpdateEffectsCount)
		{
			...
			foreach (int entry in tooltipids)
			{
				if (GameClient.CanSendTooltip(24, entry))
					SendDelveInfo(DOL.GS.PacketHandler.Client.v168.DetailDisplayHandler.DelveSpell(GameClient, entry));
			}
		}
		...
		public virtual void SendWarmapBonuses()
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarmapBonuses)))
			{
				int RealmKeeps = 0;
				int RealmTowers = 0;
				switch ((eRealm)GameClient.Player.Realm)
				{
					case eRealm.Albion:
						RealmKeeps = AlbKeeps;
        ...
        public virtual void SendUpdateMaxSpeed()
        {
            ...
            {
                GameClient.Player.LastMaxSpeed = GameClient.Player.MaxSpeed; // patch 0024 experimental hackdetect
                ...
                pak.WriteByte(
                    ...
                    Math.Min(byte.MaxValue,
                             ((GameClient.Player.MaxSpeed * 100 / GamePlayer.PLAYER_BASE_SPEED) *
                              (GameClient.Player.GetModified(eProperty.WaterSpeed) * .01))));
                SendTCP(pak);
            }
        }
        ...
        public virtual void SendCustomDialog(string msg, CustomDialogResponse callback)
        {
            ...
            {
                if (GameClient.Player.CustomDialogCallback != null)
                    GameClient.Player.CustomDialogCallback(GameClient.Player, 0x00);
        ...
        public virtual void SendCheckLOS(GameObject Checker, GameObject Target, CheckLOSResponse callback)
        {
            ...
            {
                old_callback = (CheckLOSResponse)GameClient.Player.TempProperties.getProperty<object>(key, null);
                GameClient.Player.TempProperties.setProperty(key, callback);
            }
            if (old_callback != null)
                old_callback(GameClient.Player, 0, 0); // not sure for this,  i want targetOID there
        ...
        public virtual void SendCheckLOS(GameObject source, GameObject target, CheckLOSMgrResponse callback)
        {
            ...
            lock (GameClient.Player.TempProperties)
            {
                old_callback = (CheckLOSMgrResponse)GameClient.Player.TempProperties.getProperty<object>(key, null);
                GameClient.Player.TempProperties.setProperty(key, callback);
            }
            if (old_callback != null)
                old_callback(GameClient.Player, 0, 0, 0);
        ...
        public virtual void SendExitHouse(House house, ushort unknown = 0)
        {
            ...
            if (GameClient != null && GameClient.Player != null && GameClient.ClientState != GameClient.eClientState.Linkdead)
            {
        ...
        public virtual void SendMovingObjectCreate(GameMovingObject obj)
        {
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MovingObjectCreate)))
            {
                ...
                LanguageDataObject translation = LanguageMgr.GetTranslation(GameClient, obj);
                if (translation != null)
                {
                    if (!Util.IsEmpty(((DBLanguageNPC)translation).Name))
                        name = ((DBLanguageNPC)translation).Name;
