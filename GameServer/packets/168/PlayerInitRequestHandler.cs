...
//				player.AddToWorld(); // won't work because player is already in the region
				...
				foreach (GameObject obj in player.GetInRadius(typeof(GameObject), WorldMgr.VISIBILITY_DISTANCE))
				{
					if(obj is GameSteed || obj is MovingGameStaticItem || obj is GameNPC)
					{
						player.Out.SendNPCCreate(obj);
						if (obj is GameNPC && ((GameNPC)obj).Inventory != null)
						{
							player.Out.SendLivingEquipementUpdate((GameNPC)obj);
						}
						player.CurrentUpdateArray[obj.ObjectID - 1] = true;
					}
				}
