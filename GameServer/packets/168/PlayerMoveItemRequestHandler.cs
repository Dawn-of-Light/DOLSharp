	...
	public class PlayerMoveItemRequestHandler : IPacketHandler
	{
		...
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			if(toSlot>1000)
			{
				...
				GameObject obj = client.Player.Region.GetObject(objectID) as GameObject;
				if(obj==null || obj.ObjectState!=eObjectState.Active)
				{
				...
				{
					GenericItem item = client.Player.Inventory.GetItem((eInventorySlot)fromSlot) as GenericItem;
					if(item == null)
					{
						client.Out.SendInventorySlotsUpdate(new int[] {fromSlot});
					...
					{
						client.Out.SendMessage(obj.Name+" doesn't want this item!",eChatType.CT_System,eChatLoc.CL_SystemWindow);
					}
				...
				if(fromSlot>=(ushort)eInventorySlot.Mithril && fromSlot<=(ushort)eInventorySlot.Copper)
				{
					...
					if(flatmoney > client.Player.Money)
					{
						client.Out.SendInventorySlotsUpdate(new int[] {fromSlot});
						return 0;
					}
					...
					{
						client.Out.SendMessage(obj.Name+" doesn't want your money!",eChatType.CT_System,eChatLoc.CL_SystemWindow);
						return 0;
					}
				...
				|| (toSlot>=(ushort)eInventorySlot.FirstVault && toSlot<=(ushort)eInventorySlot.LastVault)))
			{
				...
				((GamePlayerInventory)client.Player.Inventory).MoveItem((eInventorySlot)fromSlot, (eInventorySlot)toSlot, itemCount);
				return 1;
			}
			...
			if (((fromSlot>=(ushort)eInventorySlot.Ground && fromSlot<=(ushort)eInventorySlot.LastBackpack)
				|| (fromSlot>=(ushort)eInventorySlot.FirstVault && fromSlot<=(ushort)eInventorySlot.LastVault))
				&& (toSlot==(ushort)eInventorySlot.PlayerPaperDoll || toSlot==(ushort)eInventorySlot.PlayerPaperDoll178))
			{
				...
				{
					if(client.Player.Inventory.GetItem(slot) == null)
					{
						toSlot = (ushort)slot;
						break;
					}
