...
namespace DOL.GS.PacketHandler.Client.v168
{
    ...
    public class SiegeWeaponActionHandler : IPacketHandler
    {
        public void HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                case 1:
                    client.Player.SiegeWeapon.Load(ammo);
                    break; // select ammo need Log to know how sent
                case 2:
                    client.Player.SiegeWeapon.Arm();
                    break; // arm
                case 3:
                    client.Player.SiegeWeapon.Aim();
                    break; // aim
                case 4:
                    client.Player.SiegeWeapon.Fire();
                    break; // fire
                case 5:
                    client.Player.SiegeWeapon.Move();
                    break; // move
                case 6:
                    client.Player.SiegeWeapon.Repair();
                    break; // repair
                case 7:
                    client.Player.SiegeWeapon.salvage();
                    break; // salvage
                case 8:
                    client.Player.SiegeWeapon.ReleaseControl();
                    break; // release
                case 9:
                    client.Player.SiegeWeapon.StopMove();
                    break; // stop
                case 10:
                    client.Player.SiegeWeapon.Fire();
                    break; // swing
