					...
					case "findid":
						{
							...
							if (name != "")
							{
								...
								DisplayMessage(client, LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Item.FindID.MatchingIDsForX", name, items.Count), Array.Empty<object>() );
								foreach (ItemTemplate item in items)
								{
									DisplayMessage(client, item.Id_nb + " (" + item.Name + ")", Array.Empty<object>() );
								}
							}
							break;
						}
					...
					case "findname":
						{
							...
							if (name != "")
							{
								...
								DisplayMessage(client, LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Item.FindName.MatchingNamesForX", name, items.Count), Array.Empty<object>() );
								foreach (ItemTemplate item in items)
								{
									DisplayMessage(client, item.Name + "  (" + item.Id_nb + ")", Array.Empty<object>() );
								}
							}
							break;
						}
