...
namespace DOL.GS.Commands
{
    ...
    public class GMInfoCommandHandler : AbstractCommandHandler, ICommandHandler
    {
            ...
            if (client.Player.TargetObject != null)
            {
                ...
                */
                ...
                if (client.Player.TargetObject is GamePlayer)
                {
                    ...
                    info.Add(" ");
                    info.Add("  - Craftingskill : " + target.CraftingPrimarySkill + string.Empty);
                    info.Add("  - Model ID : " + target.Model);
                    info.Add("  - AFK Message: " + target.TempProperties.getProperty<string>(GamePlayer.AFK_MESSAGE) + string.Empty);
                    info.Add(" ");
        ...
        private string MaterialDoor(int material)
        {
            if (material == 0)
            {
                return " Wooden Double";
            }

            if (material == 1)
            {
                return " Wooden with Chain";
            }
