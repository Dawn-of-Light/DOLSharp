				...
				case "equipinfo":
					{
						...
						foreach (InventoryItem item in targetMob.Inventory.AllItems)
						{
							...
							client.Out.SendMessage("        Colour: " + item.Color, eChatType.CT_System, eChatLoc.CL_PopupWindow);
							...
							case "add":
								{
									if (args.Length >= 5)
									{
										try
										{
											byte slot = GlobalConstants.NameToSlot(args[3]);
											if (slot == 0)
											{
											...
											ushort model = Convert.ToUInt16(args[4]);
											byte color = 0;
											byte effect = 0;

											if (args.Length >= 6)
												byte.TryParse(args[5], out color);
											if (args.Length >= 7)
												byte.TryParse(args[6], out effect);
				...
				case "viewloot":
					{
						if (args.Length == 3 && args[2] == "random")
						{
							...
							{
								string message = string.Format("Name: {0}, Id_nb: {1}", temp.Name, temp.TemplateID);
						...
						else
						{
							...
							foreach (DBLootTemplate loot in template)
							{
								...
								if (loot.ItemTemplate == null)
									...
									message += loot.ItemTemplate.Name + " (" + loot.ItemTemplate.TemplateID + ")";
