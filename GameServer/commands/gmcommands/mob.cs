...
namespace DOL.GS.Commands
{
	...
	public class MobCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		...
		private void npctemplate(GameClient client, GameNPC targetMob, string[] args)
		{
			...
			if (template == null)
			{
				DisplayMessage(client, "No template found for " + id, Array.Empty<object>() );
				return;
			}
			...
			if (targetMob == null)
			{
				...
				mob.CurrentRegion = client.Player.CurrentRegion;
				mob.AddToWorld();
				DisplayMessage(client, "Created npc based on template " + id, Array.Empty<object>() );
			}
			else
			{
				...
				targetMob.SaveIntoDatabase();
				DisplayMessage(client, "Updated npc based on template " + id, Array.Empty<object>() );
			}
		}
