    ...
    public class TimeCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            if (client.Account.PrivLevel == (int)ePrivLevel.Admin) // admins only
            {
                ...
                catch
                {
                    client.Out.SendMessage(
                        "ADMIN Usage: /time <speed> (24 is normal, higher numbers make faster days) <time> (1 - 1000) - Reset days with new length, starting at the given time.",
            ...
            if (client.Player != null)
            {
                ...
                client.Out.SendMessage(
                    "It is " + hour.ToString() + ":" + minute.ToString("00") + ":" + seconds.ToString("00") + (pm ? " pm" : string.Empty),
                                       eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
        }
    }
