	...
	public class GameSiegeWeapon : GameMovingObject
	{
		...
		*/
		...
		public void Repair()
		{
			if (TimesRepaired <= 3)
			{
				if (Owner is GamePlayer && (Owner as GamePlayer).GetCraftingSkillValue(eCraftingSkill.WoodWorking) < 301)
				{
					(Owner as GamePlayer).Out.SendMessage("You must have woodworking skill to repair a siege weapon.", eChatType.CT_Say, eChatLoc.CL_SystemWindow);
					return;
				}
			...
			{
				if (Owner is GamePlayer)
					((GamePlayer)this.Owner).Out.SendMessage("The siegeweapon has decayed beyond repairs!", eChatType.CT_Say, eChatLoc.CL_SystemWindow);
			}
		}
		...
		public void salvage()
		{
			if (Owner is GamePlayer)
			{
				if (((GamePlayer)Owner).GetCraftingSkillValue(eCraftingSkill.SiegeCrafting) == -1)
				{
