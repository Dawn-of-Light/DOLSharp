...
{
	/// <summary>
	/// Class used for summoned GameNPC
	/// Override some stats or game mechanics to take the owner properties into account.
	/// </summary>
	public class GamePet : GameNPC
	{
		// constructor
		public GamePet(INpcTemplate template)
			: base(template)
		{
			if (Inventory != null)
			{
		...
		public override int SpellCriticalChance
		{
            get {
				...
				if(Brain is IControlledBrain)
				{
					...
					if(owner != null)
					{
						return owner.SpellCriticalChance;
					}
				}
		...
		{
			if (weapon != null && weapon.Type_Damage != 0)
			{
