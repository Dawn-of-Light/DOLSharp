...
namespace DOL.GS.Spells
{
    ...
    public class CharmSpellHandler : SpellHandler
    {
        ...
        /// <summary>
        /// Charm can have unlimited duration
        /// </summary>
        ...
        /// <summary>
        /// For applying Pulsing Charm, the creature only need to be in range...
        /// </summary>
        /// <param name="target"></param>
        /// <param name="quiet"></param>
        /// <returns></returns>
		public override bool CheckSpellPulseApply(GameLiving target, bool quiet)
		{
			// If caster is CC and cannot cast
			if (Caster.IsMezzed || Caster.IsStunned)
				return false;
			
			// If target is not in range/etc, do not notify the spell will not fail
			if (!CheckCastingTarget(ref target, quiet, false, false, false))
				return false;
		...
		/// <summary>
		/// Target is valid if it's our own charmed pet.
		/// </summary>
		/// <param name="target"></param>
		/// <param name="quiet"></param>
		...
		{
			// Charm spell are only enemy targetted spells
			if(Spell.Target.ToLower().Equals("enemy"))
			{
				...
				// special charm check
				if (!(target is GameNPC) || (target is GameNPC && !(((GameNPC)target).Brain is IControlledBrain) && target.Realm != 0) || (target is GameNPC && (((GameNPC)target).Flags & GameNPC.eFlags.PEACE) != 0))
	            {
	    	    ...
	    	    if(((GameNPC)target).Brain != null && ((GameNPC)target).Brain is IControlledBrain && ((IControlledBrain)((GameNPC)target).Brain).Owner != Caster)
	            {
	                MessageToCaster("Your target is already under control.", eChatType.CT_SpellResisted);
	                return false;
	            }
				...
				if (!Caster.IsWithinRadius(target, CalculateSpellRange()))
				{
					if (!quiet)
					{
						if(needViewOffensive)
						{
							MessageToCaster("That target is too far away!", eChatType.CT_SpellResisted);
						}
						else
						{
							MessageToCaster("Your charmed creature is too far away!", eChatType.CT_SpellResisted);
						}
					}
				...
				if (needViewOffensive && Caster.TargetObject == target && !Caster.TargetInView)
				{
					if (!quiet)
        ...
        public override bool CheckEndCast(GameLiving target)
        {
            ...
            if (Spell.IsPulsing && Caster.ControlledBrain != null && Caster.ControlledBrain.Body == target)
            {
            	GameEventMgr.Notify(GameLivingEvent.PetReleased, target);
            }
        ...
        public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
        {
            ...
            if (Caster is GamePlayer)
            {
                ...
                if (Spell.IsPulsing) // not permanent
                {
        ...
        public override void OnEffectStart(GameSpellEffect effect)
        {
            ...
            if (player != null && npc != null)
            {
                ...
                if (m_controlledBrain == null || Caster.ControlledBrain != m_controlledBrain)
                ...
                if (player.ControlledBrain != m_controlledBrain)
                {
	                ...
	                foreach (GamePlayer plyer in npc.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE)) 
	                {
	                	GamePlayer ply = plyer;
                ...
                // TODO Bad Idea to Inline Create Packet and Update Packet
                // npc.BroadcastUpdate();
        ...
        private void ReleaseEventHandler(DOLEvent e, object sender, EventArgs arguments)
        {
            ...
            PulsingSpellEffect concEffect = SpellHelper.FindPulsingSpellOnTarget(npc.Owner, this);
            if (concEffect != null)
                concEffect.Cancel(false);

            GameSpellEffect charm = SpellHelper.FindEffectOnTarget(npc.Body, this);
                ...
                if (!noMessages) // no overwrite
                {
                    ...
                    lock (npc.BrainSync)
                    {
                        if (npc.Brain != null && npc.Brain is IOldAggressiveBrain)
                        {
                            ...
                            if (Spell.IsPulsing && Caster.ObjectState == GameObject.eObjectState.Active && Caster.IsAlive)
                            {
                    ...
                    lock (((ICollection)npc.Attackers).SyncRoot)
                    {
	                    foreach (GameObject attker in npc.Attackers)
	                    {
	                    	GameObject obj = attker;
                    ...
                    // TODO Stop attack already do that
                    //npc.StopFollowing();
        ...
        {   
            return (neweffect.SpellHandler is CharmSpellHandler && oldeffect.SpellHandler is CharmSpellHandler && oldeffect.SpellHandler.Caster == neweffect.SpellHandler.Caster);
        }
        ...
        {
            base.SendEffectAnimation(m_charmedNpc, clientEffect, boltDuration, noSound, success);
        }
