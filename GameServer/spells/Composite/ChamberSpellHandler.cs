...
namespace DOL.GS.Spells
{
	...
	public class ChamberSpellHandler : SpellHandler
	{
		...
		public override bool InterruptCasting()
		{
			if(base.InterruptCasting())
			{
		...
		public override bool CastSpell()
		{
			...
			GameSpellEffect effect = SpellHelper.FindEffectOnTarget(caster, "Chamber", m_spell.Name);
			if(effect != null && m_spell.Name == effect.Spell.Name)
			{
				...
				GameSpellEffect PhaseShift = SpellHelper.FindEffectOnTarget(SpellCastTarget, "Phaseshift");
				...
				if (caster.IsObjectInFront(SpellCastTarget, 180) == false)
				{
				...
				if (SpellCastTarget is GamePlayer)
				{
					if ((SpellCastTarget as GamePlayer).IsInvulnerableToAttack)
					{
						MessageToCaster("Your target is invunerable at the momment and cannot be attacked!", eChatType.CT_System);
						return false;
					}
				}
				if (!caster.IsWithinRadius(SpellCastTarget, ((SpellHandler)spellhandler).CalculateSpellRange()))
				{
				...
				if (PhaseShift != null)
				{
					MessageToCaster(SpellCastTarget.Name + " is Phaseshifted and can't be attacked!", eChatType.CT_System); return false;
				}
				if (SelectiveBlindness != null)
				{
					GameLiving EffectOwner = SelectiveBlindness.EffectSource;
					if (EffectOwner == SpellCastTarget)
					{
						if (m_caster is GamePlayer)
							((GamePlayer)m_caster).Out.SendMessage(string.Format("{0} is invisible to you!", SpellCastTarget.GetName(0, true)), eChatType.CT_Missed, eChatLoc.CL_SystemWindow);

						return false;
					}
				}

				if (GameServer.ServerRules.IsAllowedToAttack(Caster, SpellCastTarget, true) && chamber.PrimarySpell.Target.ToLower() == "realm")
				{
		...
		public override void FinishSpellCast(GameLiving target)
		{
			...
			if (m_caster is GamePlayer)
			{
				...
				if (quickcast != null && Spell.CastTime > 0)
				{
					...
					m_caster.DisableSkill(m_caster.GetAbility(Abilities.Quickcast), QuickCastAbilityHandler.DISABLE_DURATION);
