...
#region TODO
/*	[SpellHandlerAttribute("PhaseShift")]
	public class PhaseShiftSpellHandler : SpellHandler
	{
		public override void OnEffectStart(GameSpellEffect effect)
		{	
			base.OnEffectStart(effect);
			...
			{			
				GamePlayer targetPlayer = effect.Owner as GamePlayer;
				targetPlayer.SetPvPInvulnerability(m_spell.Duration,null);
				targetPlayer.StopAttack();
				targetPlayer.StopCurrentSpellcast();
				targetPlayer.DisableTurning(true);
				targetPlayer.IsStunned = true;
				targetPlayer.EffectList.Add(effect);
		...
		{		
			...
			{			
				GamePlayer targetPlayer = effect.Owner as GamePlayer;
				targetPlayer.DisableTurning(false);
				targetPlayer.IsStunned = false;
				targetPlayer.EffectList.Remove(effect);
				targetPlayer.Out.SendUpdateMaxSpeed();
			}
			return base.OnEffectExpires(effect, noMessages);
		}
		        
		public PhaseShiftSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line){}
	}*/
#endregion