...
namespace DOL.GS.Spells
{
	...
	public class ResurrectSpellHandler : SpellHandler
	{
		...
		// constructor
		public ResurrectSpellHandler(GameLiving caster, Spell spell, SpellLine line)
			: base(caster, spell, line)
		{
		}
		
		public virtual byte IllnessReduction
		{
			get { return (byte)Math.Max(0, (((Spell.ResurrectHealth-10) / 90) * 50)); }
		}
			...
			if (!player.IsAlive)
			{
				...
				else
				{
					...
					else
					{
						...
						//refund mana
						m_caster.Mana += PowerCost(player, true);
					}
				}
			}
		...
		protected virtual void ResurrectLiving(GameLiving living)
		{
			...
			living.Health = (int)(living.MaxHealth * m_spell.ResurrectHealth * 0.01);
			int tempManaEnd = (int)(m_spell.ResurrectMana * 0.01);
			...
			if (player != null)
			{
				...
				player.Notify(GamePlayerEvent.Revive, player, new RevivedEventArgs(Caster, this, IllnessReduction));
                
                //Lifeflight add this should make it so players who have been ressurected don't take damage for 5 seconds
                StartResurrectImmunity(player);
		...
		protected virtual void StartResurrectImmunity(GameLiving target)
		{
			if(target is GamePlayer)
			{
				RezDmgImmunityEffect rezImmune = new RezDmgImmunityEffect();
				rezImmune.Start(target);
			}
		}
		...
		public override bool CheckBeginCast(GameLiving target)
		{
            ...
            if (m_caster.Mana < PowerCost(target, true))
            {
