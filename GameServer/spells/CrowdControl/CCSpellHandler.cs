...
namespace DOL.GS.Spells
{
	...
	public abstract class AbstractCCSpellHandler : ImmunityEffectSpellHandler
	{
		...
		public override int CalculateEffectDuration(GameLiving target, double effectiveness)
		{
			...
			// reduce duration if GameNPC has immunity timer
			if (target is GameNPC)
			{
				GameSpellEffect npcEffect = SpellHelper.FindImmunityEffectOnTarget(target, GetType());
		...
		public override int CalculateSpellResistChance(GameLiving target)
		{
			int resistvalue = 0;
			int resist = 0;
			GameSpellEffect fury = SpellHelper.FindEffectOnTarget(target, "Fury");
			if (fury != null)
			{
		...
		public AbstractCCSpellHandler(GameLiving caster, Spell spell, SpellLine line)
	...
	public class MesmerizeSpellHandler : AbstractCCSpellHandler
	{
		...
		public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
		{
			if (target.HasAbility(Abilities.MezzImmunity) || SpellHelper.FindStaticEffectOnTarget(target, typeof(MezzRootImmunityEffect)) != null)
			{
				MessageToCaster(target.Name + " is immune to this effect!", eChatType.CT_SpellResisted);
				SendEffectAnimation(target, 0, false, 0);
				target.StartInterruptTimer(target.SpellInterruptDuration, AttackData.eAttackType.Spell, Caster);
			...
			GameSpellEffect mezblock = SpellHelper.FindEffectOnTarget(target, "CeremonialBracerMezz");
			if (mezblock != null)
			{
		...
		// constructor
		public MesmerizeSpellHandler(GameLiving caster, Spell spell, SpellLine line)
			: base(caster, spell, line)
		{
		}
	}
	...
	public class StunSpellHandler : AbstractCCSpellHandler
	{
		...
		public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
		{
			...
			//use AmnesiaChance>0 to calculate stun immunity timer (such pet stun spells), actually (1.90) pet stun immunity is 5x the stun duration
			//AmnesiaChance negative means no immunity !
			if(Spell.AmnesiaChance > 0)
			{
				base.OnEffectExpires(effect, noMessages);
				return Spell.Duration * Spell.AmnesiaChance;
			}
			else if (Spell.AmnesiaChance < 0)
			{
				base.OnEffectExpires(effect, noMessages);
				return 0;
			}
		...
		public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
		{
			...
			if(Spell.SpellType.ToLower() != "stylestun" )
			{
				GameSpellEffect stunblock = SpellHelper.FindEffectOnTarget(target, "CeremonialBracerStun");
				if (stunblock != null)
				{
