...
namespace DOL.GS.Spells
{
	...
	public class UnbreakableSpeedDecreaseSpellHandler : ImmunityEffectSpellHandler
	{
		...
		{
			if (target.HasAbility(Abilities.CCImmunity) || target.HasAbility(Abilities.RootImmunity) || SpellHelper.FindStaticEffectOnTarget(target, typeof(MezzRootImmunityEffect)) != null)
			{
		...
		private sealed class RestoreSpeedTimer : GameTimer
		{
			...
			protected override void OnTick()
			{
				...
				{
					// No duration Effect (unlimited) should have a standard factor.
					factor = 1.0;
				}
				else
				{
					factor = 2.0 - (effect.Duration - effect.RemainingTime)/(double)(effect.Duration>>1);
				}
