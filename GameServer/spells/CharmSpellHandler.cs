...
namespace DOL.GS.Spells
{
	...
	public class CharmSpellHandler : SpellHandler
	{
		...
		/// <summary>
		/// Holds the charmed mob for pulsing spells
		/// </summary>
		protected GameLivingBase m_charmTarget;
		...
		{
			Caster.ChangeMana(null, -CalculateNeededPower(target));
		...
		/// <returns></returns>
		public override IList SelectTargets(GameLivingBase castTarget)
		{
			if (m_charmTarget == null) m_charmTarget = castTarget; // save target on first start
			
			ArrayList list = new ArrayList(1);
			list.Add(m_charmTarget);
			return list;	
		}
		...
		public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
		{
			if (m_controlledBrain == null) // check only if brain wasn't changed at least once
			{
				...
				if (target.Realm != 0 || target is GameMob == false)
				{
...
//					GameEventMgr.AddHandler(npc, GameLivingBaseEvent.Dying, new DOLEventHandler(ReleaseEventHandler)); // must be canceled by Die() method anyway, makes some problems with attackers list this way
				}
				...
				{
					foreach(GamePlayer ply in npc.GetInRadius(typeof(GamePlayer), WorldMgr.VISIBILITY_DISTANCE))
		...
		private void ReleaseEventHandler(DOLEvent e, object sender, EventArgs arguments)
		{
			...
			else if (e == GameLivingBaseEvent.Dying)
...
//					GameEventMgr.RemoveHandler(npc, GameLivingBaseEvent.Dying, new DOLEventHandler(ReleaseEventHandler));
					...
					foreach (GameLiving obj in attackers)
					{
					...
					npc.TempProperties.setProperty(GameMob.CHARMED_TICK_PROP, npc.Region.Time);
				}
			}
			else
			{
