...
namespace DOL.GS.Database
{
	...
	public class ObjectDatabase
	{
		...
		/// The conctructor
		/// </summary>
		public ObjectDatabase(string configurationFile)
		{
		...
		public bool TestDatabaseStructure(string configurationFile, IList errors)
		{
			if(m_databaseConfiguration != null)
			{
				...
				foreach(PersistentClass pClass in m_databaseConfiguration.ClassMappings)
				{
					...
					{
						errors.Add(String.Format("Table: `{0}` is missing!", requiredTablename));
						dbOK = false;
					}
					else
					{
						...
						foreach(Column requiredColumn in pClass.Table.ColumnCollection)
						{
							...
							{								
								errors.Add(String.Format("Table: `{0}` Column: `{1}` Type: {2} is missing", requiredTablename, requiredColumn.Name, requiredColumn.GetSqlType(Dialect.GetDialect(m_databaseConfiguration.Properties), null)));
								dbOK = false;
							}
						}	
					}
