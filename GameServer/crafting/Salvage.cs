...
namespace DOL.GS
{
	...
	public class Salvage
	{
		...
		protected static int Proceed(RegionTimer timer)
		{
			...
			int oldCount = newItem.Count;
			newItem.Count = materialCount;
			newItem.Weight = (int)(newItem.Count * ((float)newItem.Weight / oldCount));
			newItem.Value = (long)(newItem.Count * ((float)newItem.Value / oldCount));

			player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, newItem);
				
			player.Out.SendMessage("You get back " +materialCount+ " "+ newItem.Name+ " after salvaging the " +itemToSalvage.Name+ ".",eChatType.CT_Important,eChatLoc.CL_SystemWindow);
