...
namespace DOL.GS.ServerRules
{
    public abstract class AbstractServerRules : IServerRules
    {
        ...
        public virtual void ImmunityExpiredCallback(GamePlayer player)
        {
            ...
            if (player.Client.IsPlaying == false)
            {
                return;
            }
        ...
        public virtual bool IsAllowedToAttack(GameLiving attacker, GameLiving defender, bool quiet)
        {
            ...
            if (playerAttacker != null)
            {
                if (defender is GameNPC && playerAttacker.IsInvulnerableToAttack)
                {
                    if (quiet == false)
                    {
                        MessageToLiving(attacker, "You can't attack until your PvP invulnerability timer wears off!");
                    }
            ...
            foreach (AbstractArea area in defender.CurrentAreas)
            {
                ...
                if (defender is GamePlayer)
                {
                    if (quiet == false)
                    {
                        MessageToLiving(attacker, "You can't attack someone in a safe area!");
                    }
        ...
        public virtual string ReasonForDisallowMounting(GamePlayer player)
        {
            ...
            if (!player.IsAlive)
            {
                return "GamePlayer.UseSlot.CantMountWhileDead";
            }

            if (player.Steed != null)
            {
                return "GamePlayer.UseSlot.MustDismountBefore";
            }
            ...
            if (player.IsMoving)
            {
                return "GamePlayer.UseSlot.CantMountMoving";
            }

            if (player.InCombat)
            {
                return "GamePlayer.UseSlot.CantMountCombat";
            }

            if (player.IsSitting)
            {
                return "GamePlayer.UseSlot.CantCallMountSeated";
            }

            if (player.IsStealthed)
            {
                return "GamePlayer.UseSlot.CantMountStealthed";
            }
            ...
            lock (killedNPC.XPGainers.SyncRoot)
            {
                ...
                foreach (DictionaryEntry de in killedNPC.XPGainers)
                {
                    ...
                    else
                    {
                        ...
                        if (killedNPC.CampBonus >= 0.05)
                        {
                            killedNPC.CampBonus -= 0.05; // decrease camp bonus by 5% per kill
                        }
                    }
