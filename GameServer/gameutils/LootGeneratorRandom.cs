            ...
            if (m_itemTemplates == null)
            {
                ...
                lock (m_itemTemplates.SyncRoot)
                {
                    ...
                    for (int i = 0; i <= LEVEL_SIZE; i++)
                    {
                        try
                        {
                            itemTemplates = GameServer.Database.SelectObjects<ItemTemplate>(
                                "`Level` >= @LevelMin AND `Level` <= @LevelMax AND `IsPickable` = @IsPickable AND `IsDropable` = @IsDropable AND `CanDropAsloot` = @CanDropAsloot",
