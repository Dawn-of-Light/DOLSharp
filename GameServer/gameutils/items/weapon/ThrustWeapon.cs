...
{
	/// <summary>
	/// Summary description for a ThrustWeapon
	/// </summary> 
	public class ThrustWeapon : Weapon
	{
		...
		public override eDamageType DamageType 
		{
			get
			{
				// ThrustWeapon can make damage of type : 
				...
				if(m_damageType != eDamageType.Matter && m_damageType != eDamageType.Spirit && m_damageType != eDamageType.Heat && m_damageType != eDamageType.Cold)
					if(m_damageType != eDamageType.Thrust) m_damageType = eDamageType.Thrust;
				return m_damageType;
			}
			set
			{
		...
		{
			get	// ThrustWeapon can only be right or left hand
			{
		...
		{
			get { return eObjectType.ThrustWeapon; }
		}
	}
}
