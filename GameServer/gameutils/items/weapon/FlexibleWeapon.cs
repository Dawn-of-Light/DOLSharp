...
{
	/// <summary>
	/// Summary description for a FlexibleWeapon
	/// </summary> 
	public class FlexibleWeapon : Weapon
	{
		/// <summary>
		...
		public override eDamageType DamageType 
		{
			get
			{
				// FlexibleWeapon can make damage of type : 
				// - legendary weapon : Matter, Spirit, Heat, Cold
				// - classic weapon : Crush, Slash
				if(m_damageType != eDamageType.Matter && m_damageType != eDamageType.Spirit && m_damageType != eDamageType.Heat && m_damageType != eDamageType.Cold)
					if(m_damageType != eDamageType.Crush && m_damageType != eDamageType.Slash) m_damageType = eDamageType.Crush;
				return m_damageType;
			}
			set
			{
		...
		{
			// FlexibleWeapon can only be right hand
		...
		{
			get { return new eInventorySlot[] {eInventorySlot.RightHandWeapon, eInventorySlot.TwoHandWeapon }; }
		}
		...
		{
			get { return eObjectType.FlexibleWeapon; }
		}
	}
}
