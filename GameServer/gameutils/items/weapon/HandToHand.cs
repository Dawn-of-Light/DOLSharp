...
{
	/// <summary>
	/// Summary description for a HandToHand
	/// </summary> 
	public class HandToHand : Weapon
	{
		...
		public override eDamageType DamageType 
		{
			get
			{
				// HandToHand can make damage of type : 
				// - legendary weapon : Matter, Spirit, Heat, Cold
				// - classic weapon : Slash, Thrust
				if(m_damageType != eDamageType.Matter && m_damageType != eDamageType.Spirit && m_damageType != eDamageType.Heat && m_damageType != eDamageType.Cold)
					if(m_damageType != eDamageType.Slash && m_damageType != eDamageType.Thrust) m_damageType = eDamageType.Slash;
				return m_damageType;
			}
			set
			{
		...
		{
			// HandToHand can only be left hand
		...
		{
			get { return eObjectType.HandToHand; }
		}
	}
}
