...
namespace DOL.GS
{
	...
	public class Ability : NamedSkill
	{
		...
		public Ability(DBAbility dba, int level)
			: this(dba.KeyName, dba.Name, dba.Description, dba.ID, (ushort)dba.IconID, level)
		{
		}

		public Ability(DBAbility dba, int level, string spec, int speclevel)
			: this(dba.KeyName, dba.Name, dba.Description, dba.ID, (ushort)dba.IconID, level, spec, speclevel)
		{
		}
