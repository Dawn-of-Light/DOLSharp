...
namespace DOL.GS.Appeal
{
    public static class AppealMgr
    {
        ...
        {
            DBAppeal appeal = (DBAppeal)GameServer.Database.SelectObject(typeof(DBAppeal), "`Name` = '" + GameServer.Database.Escape(name) + "'");
            return appeal;
        }
        ...
        {
            List<DBAppeal> rlist = new List<DBAppeal>();
            DBAppeal[] appeal = (DBAppeal[])GameServer.Database.SelectAllObjects(typeof(DBAppeal));
            if (appeal != null)
            {
                rlist.AddRange(appeal);
            }
            return rlist;
        }
        /// <summary>
