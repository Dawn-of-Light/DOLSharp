            ...
            lock (m_items)
            {
                try
                {
                    ...
                    var items = GameServer.Database.SelectObjects<InventoryItem>(
                        "`OwnerID` = @OwnerID AND (`SlotPosition` <= @LastVault OR (`SlotPosition` >= @OtherMin AND `SlotPosition` < @OtherMax))",
                    ...
                    foreach (InventoryItem item in items)
                    {
                        ...
                        {
                            Log.Error(
                                "Error loading player inventory (" + inventoryID + "), Inventory_ID: " +
                                      item.ObjectId +
                                      " (" + (item.ITemplate_Id == null ? string.Empty : item.ITemplate_Id) +
                                      ", " + (item.UTemplate_Id == null ? string.Empty : item.UTemplate_Id) +
                    ...
                    switch (toSlot)
                    {
                        ...
                        case eInventorySlot.HorseBarding:
                            if (fromItem.Item_Type != (int)eInventorySlot.HorseBarding)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                                    "You can't put " + fromItem.GetName(0, true) + " in your active barding slot!",
                                                         eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
                        ...
                        case eInventorySlot.HorseArmor:
                            if (fromItem.Item_Type != (int)eInventorySlot.HorseArmor)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                                    "You can't put " + fromItem.GetName(0, true) + " in your active horse armor slot!",
                                                         eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
                        ...
                        case eInventorySlot.Horse:
                            if (fromItem.Item_Type != (int)eInventorySlot.Horse)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                                    "You can't put " + fromItem.GetName(0, true) + " in your active mount slot!",
                                                         eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
                    ...
                    switch (fromSlot)
                    {
                        ...
                        case eInventorySlot.HorseBarding:
                            if (toItem.Item_Type != (int)eInventorySlot.HorseBarding)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                                    "You can't put " + toItem.GetName(0, true) + " in your active barding slot!",
                                                         eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
                        ...
                        case eInventorySlot.HorseArmor:
                            if (toItem.Item_Type != (int)eInventorySlot.HorseArmor)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                                    "You can't put " + toItem.GetName(0, true) + " in your active horse armor slot!",
                                                         eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
                        ...
                        case eInventorySlot.Horse:
                            if (toItem.Item_Type != (int)eInventorySlot.Horse)
                            {
                                valid = false;
                                m_player.Out.SendMessage(
                                    "You can't put " + toItem.GetName(0, true) + " in your active mount slot!",
                                                         eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            }
