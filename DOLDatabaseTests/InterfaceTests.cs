...
namespace DOL.Database.Tests
{
    ...
    public class InterfaceTests
    {
        ...
        public void TestMultipleSaveDifferentTable()
        {
            ...
            foreach (var obj in retrieved.Where(o => o.GetType() == typeof(TestTableRelations)).OfType<TestTableRelations>())
            {
                CollectionAssert.AreEquivalent(
                    objsTestTableRelations.First(o => o.ObjectId == obj.ObjectId).Entries.Select(o => o.ObjectId),
        ...
        {
            Assert.Throws(typeof(DatabaseException), () => Database.FindObjectByKey<TableNotRegistered>(1), string.Empty);
        }
        ...
        public void TestSelectObjects()
        {
            ...
            var nullWhere = Database.SelectObjects<TestTable>(string.Empty, new QueryParameter[] { });
        ...
        public void TestSelectObjectsWithMultipleWhereClause()
        {
            ...
            foreach (var sets in objectByGroup)
            {
                ...
                Assert.IsTrue(
                    sets.Objects.All(obj => obj.TestField.Equals(sets.Grp, StringComparison.OrdinalIgnoreCase)),
            ...
            CollectionAssert.AreEqual(new [] { string.Empty, string.Empty }.Concat(orderedObjs.Select(obj => obj.ObjectId.ToLower())), resultsManyWithMissing.Select(obj => obj != null ? obj.ObjectId.ToLower() : string.Empty),
                                      "Retrieve Sets from Select Objects should be Equal to Parameter Set ObjectId...");
            CollectionAssert.AreEqual(new [] { string.Empty, string.Empty }.Concat(orderedObjs.Select(obj => obj.TestField.ToLower())), resultsManyWithMissing.Select(obj => obj != null ? obj.TestField.ToLower() : string.Empty),
                                      "Retrieve Sets from Select Objects should be Equal to Parameter Set Field Value...");
        }
        ...
        public void TestSelectObjectsWithNull()
        {
            ...
            CollectionAssert.AreEquivalent(allobjects.Select(obj => obj.ObjectId), retrieveMultipleNullWithIsolation.Select(obj => obj.ObjectId), string.Empty);

            var retrieveParameter = Database.SelectObjects<TestTable>(null, new QueryParameter());

            CollectionAssert.AreEquivalent(allobjects.Select(obj => obj.ObjectId), retrieveParameter.Select(obj => obj.ObjectId), string.Empty);

            var retrieveParameters = Database.SelectObjects<TestTable>(null, new QueryParameter[] { });

            CollectionAssert.AreEquivalent(allobjects.Select(obj => obj.ObjectId), retrieveParameters.Select(obj => obj.ObjectId), string.Empty);

            var retrieveMultipleParameters = Database.SelectObjects<TestTable>(null, new [] { new QueryParameter[] { } });

            CollectionAssert.AreEquivalent(allobjects.Select(obj => obj.ObjectId), retrieveMultipleParameters.First().Select(obj => obj.ObjectId), string.Empty);

            Assert.Throws(typeof(ArgumentNullException), () => Database.SelectObjects<TestTable>(null, (QueryParameter)null), string.Empty);

            Assert.Throws(typeof(ArgumentNullException), () => Database.SelectObjects<TestTable>(null, (IEnumerable<QueryParameter>)null), string.Empty);

            Assert.Throws(typeof(ArgumentNullException), () => Database.SelectObjects<TestTable>(null, (IEnumerable<IEnumerable<QueryParameter>>)null), string.Empty);

            Assert.Throws(typeof(NullReferenceException), () => Database.SelectObjects<TestTable>(null, new QueryParameter[] { null }), string.Empty);

            Assert.Throws(typeof(NullReferenceException), () => Database.SelectObjects<TestTable>(null, new [] { new QueryParameter[] { null } }), string.Empty);

            Assert.Throws(typeof(ArgumentNullException), () => Database.SelectObjects<TestTable>(null, new QueryParameter[][] { null }), string.Empty);
