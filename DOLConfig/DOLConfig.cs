...
namespace DOLConfig
{
    public partial class DolConfig : Form
    {
        ...
        {
            ExtraPropertiesEditor property_editor = new ExtraPropertiesEditor(string.Empty, string.Empty, string.Empty, string.Empty);
            DialogResult result = property_editor.ShowDialog();
            if (result == DialogResult.OK)
            {
                // Add a new Row
                DOLConfigParser.addExtraOptionsRow(extraOptions, property_editor.propertyName, property_editor.propertyType, (object)property_editor.propertyValue, string.Empty);
            }
        }
